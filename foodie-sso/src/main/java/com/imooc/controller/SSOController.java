...
public class SSOController {
     ...
     * CAS统一登录接口
     *  目的：
     *      1. 登录后创建用的全局会话                      ->   uniqueToken
     *      2. 创建用户全局门票，用于表示在CAS端是否登录     ->   userTicket
     *      3. 创建用户的临时票数，用于回跳回传             ->   tmpTicket
     * @param username
     * @param password
     * @param returnUrl
     * @param model
    ...
    public String doLogin(String username,String password, String returnUrl, Model model,
        ...
        //0. 判断用户名与密码必须不能为空
        if (StringUtils.isBlank(username) || StringUtils.isBlank(password) || StringUtils.isBlank(password)){
        ...
        //4. userTicket需要关联用户id，并且放入到redis中，这代表这个用户有门票了，可以在各个景区游玩
        redisOperator.set(REDIS_USER_TICKET + ":" + userTicket, userResult.getId());

        //5. 生成临时票据，回跳到调用端网站，是由CAS端锁签发的一个一次性的临时Ticket
        String tmpTicket = createTmpTicket();
        ...
        /**
         * 举例：
         *      我们去动物园玩耍，大门口买了一张统一门票，这个就是CAS系统的全局门票和用户全局会话
         *      动物园有一些小的景点，需要凭你的门票去领取一次性的票据，有了这张票据以后就能去这些小景点游玩了
         *      这样的一个个小景点其实就是我们这里所对应的一个个的站点
         *      当我们使用完毕这张临时票据后，就需要销毁
         */
//        return "login";
        return "redirect:" + returnUrl + "?tmpTicket=" + tmpTicket;
    }
    ...
    public IMOOCJSONResult verifyTmpTicket(String tmpTicket, HttpServletRequest request, HttpServletResponse response) throws Exception{
        // 使用一次性临时票据来验证用户是否登录，如果登录过，把用户会话信息回传给站点
        //使用完毕后，需要销毁临时票据
        ...
        // 0. 如果临时票据ok，则需要销毁并且拿到CAS端cookie中的全局userTicket，以此获取用户全局会话
        ...
        String userId = redisOperator.get(REDIS_USER_TICKET + ":" + userTicket);
        if (StringUtils.isBlank(userId)){
            return IMOOCJSONResult.errorTokenMsg("用户票据异常");
        }
    ...
    private String getCookie(HttpServletRequest request, String key){
        ...
        if (null == cookieList || StringUtils.isBlank(key)){
            return null;
        }
